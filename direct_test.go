...
func TestVerifyPeerCertificate(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name:          "should return an error when failing to verify the certificate for masquerade and sni",
			givenRawCerts: rawCerts,
			givenRoots:    trustedCACerts(t),
			givenDomain:   "cloudfront.net",
			givenSNI:      "google.com",
			assert: func(t *testing.T, err error) {
				if assert.Error(t, err) {
					assert.ErrorContains(t, err, "certificate verification failed for masquerade and SNI")
				}
			},
		},
		{
			...
			givenRawCerts: rawCerts,
			givenRoots:    trustedCACerts(t),
			givenDomain:   "potato.akamaihd.net",
			givenSNI:      "github.com",
			assert: func(t *testing.T, err error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := verifyPeerCertificate(tt.givenRawCerts, tt.givenRoots, tt.givenDomain, tt.givenSNI)
			tt.assert(t, err)
		})
	}
}
